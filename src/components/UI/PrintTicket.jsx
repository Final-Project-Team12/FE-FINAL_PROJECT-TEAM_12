...
const PrintTicket = () => {
  ...
  return (
    <div>
      <h1>Status Cetak Tiket</h1>
      {loading && <p>Loading...</p>}
      {error && <p style={{ color: 'red' }}>Error: {error.message}</p>}
      {!loading && printTicket && (
        <>
          <p className="text-lg font-bold text-green-900">
            Name : {printTicket.data.ticket_details.passenger_name}
          </p>
          <p className="text-lg font-bold text-green-900">
            Seat Number : {printTicket.data.ticket_details.seat_number}
          </p>
        </>
      )}
    </div>
    ...
    //         </span>
    //         <span className="text-xs md:text-sm">Flight Number: PLANE-1</span>
    //         <span className="text-xs md:text-sm">
    //           Passenger: Tegar Alfa Rizzi
    //         </span>
    //         <span className="text-xs md:text-sm">Class: Economy</span>
